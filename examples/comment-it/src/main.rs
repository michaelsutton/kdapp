...
use comment_it::core::episode::SimpleAuth;
use comment_it::core::commands::AuthCommand;
use comment_it::{AuthServerConfig, run_auth_server};
use comment_it::wallet::get_wallet_for_command;
use comment_it::api::http::organizer_peer::run_http_peer;

use comment_it::cli::commands::test_api_flow::TestApiFlowCommand;
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let matches = Command::new("comment-it")
        .version("0.1.0")
    ...
    match matches.subcommand() {
        ...
        Some(("test-api", sub_matches)) => {
            let peer_url = sub_matches.get_one::<String>("peer").unwrap().clone();
            let command = comment_it::cli::commands::test_api::TestApiCommand { 
                peer: peer_url, 
                verbose: false, 
                json: false 
            };
        ...
        Some(("unified-peer", sub_matches)) => {
            ...
            let organizer = comment_it::organizer::CommentOrganizer::new(host, port).await?;
            organizer.run().await?;
        }
        _ => {
